...
export default function CompanyButton({RouterHandler,name,path}:CompanyButtonProps)
{
    return(
        ...
        >
            <div className={`${path? 'grid grid-cols-[2fr_7fr_1fr]':''} w-full font-bold`}>
                {path&&<div className="text-white text-left m-auto w-[50%]">
                    <img src={path}></img>
                </div>}
                <div className="text-white">
                    {name}
                </div>
                {path&&<div className="text-white text-right pr-8">
                    &gt;
                </div>}
